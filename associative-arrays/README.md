# Домашнее задание к лекции 2.3 Ассоциативные массивы
...
В прошлом домашнем задании мы создали массив сообщений в чате. Этот массив состоял из вложенных массивов. Для печати нам нужно помнить, что под индексом `0` хранится `автор`, а под индексом `1` — `текст сообщения`. Это неудобно. Замените эти вложенные массивы на объекты.
...
2. С помощью цикла измените вложенные массивы на объекты с подходящими парами ключ-значение

3. Напечатайте все сообщения в чате по шаблону:
...
Для этого мы добавим новый ключ `chats` в объект `user`, а в качестве значения создадим новый ассоциативный массив. В этом массиве в качестве ключа будем использовать `имя собеседника`, а в качестве значения — `массив сообщений`.
...
3. Добавьте первый чат в список чатов. В качестве ключа используйте строку `'wife'`, в качестве значений массив сообщений:
...
4. Напечатайте сообщения чата из c `'wife'`
